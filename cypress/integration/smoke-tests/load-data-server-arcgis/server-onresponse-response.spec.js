...
describe('Add ARCGIS server', function() {
  before('Login', function() {
    ...
    // *********** START TEST CODE ***********
    // CYPRESS INDICATES CORRECT RESPONSE USED (CLICK REQUEST IN LOG AND VIEW CONSOLE OUTPUT), BUT
    // BROWSER TOOLS INDICATE DUMMY RESPONSE USED.  SECOND REQUEST NEVER SENT.
    var query1;
    var query2;
    cy.fixture('/smoke-tests/load-data-server-arcgis/query-1.stub.json').as('getFeatureList').then(function(query) {
      query1 = query;
    });
    cy.fixture('/smoke-tests/load-data-server-arcgis/query-2.stub.json').as('getFeatureDetails').then(function(query) {
      query2 = query;
    });
    ...
    cy.server({
      onResponse: function(xhr) {
        if ((xhr.request.body).includes('geometryType')) {
          xhr.response.body = query1;
        } else if ((xhr.request.body).includes('objectIds')) {
          xhr.response.body = query2;
        }
      }
    });

    cy.route('POST', '**/OpenData/MapServer/5/query', 'fx:/smoke-tests/load-data-server-arcgis/dummy.stub.txt')
        .as('dummy'); // WITHOUT THIS LINE, REQUESTS ARE NEVER ROUTED THROUGH CY.SERVER
  ...
  it('Load data from ARCGIS server', function() {
    ...
    cy.get(os.layersDialog.Tabs.Areas.Tree.AREA_1).rightClick();
     ...
     // THIS LINE CAUSES TWO REQUESTS TO BE SENT
    ...
    // ORIGINAL TEST CODE, CAN BE ENABLED AFTER THIS IS WORKING
    // cy.get(os.layersDialog.Tabs.Layers.Tree.LAYER_4)
    //     .find(os.layersDialog.Tabs.Layers.Tree.Type.featureLayer.FEATURE_COUNT_TEXT_WILDCARD)
    //     .invoke('text')
